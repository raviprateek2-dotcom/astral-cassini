...
| 2.7   | Frontend route guards                   | **Done**              | A+H   | `src/middleware.ts` redirects unauthenticated users; `useAuth` gate only while **`/api/auth/me`** loads on protected routes; Playwright: direct `/jobs` → `/?next=`. Deployed demos: backend **`ALLOW_SEED_DEMO_USERS_OUTSIDE_DEV`** + **`SEED_DEMO_USERS`** + **`DEMO_*_PASSWORD`** (see README). **H:** spot-check other deep links. |
