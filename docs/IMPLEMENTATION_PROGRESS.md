...
| 2.7   | Frontend route guards                   | **Done**              | A+H   | `middleware.ts` redirects unauthenticated users; `useAuth` gate only while `**/me`** loads on protected routes; Playwright: direct `/jobs` → `/?next=`. **H:** spot-check other deep links.                                                                  |
